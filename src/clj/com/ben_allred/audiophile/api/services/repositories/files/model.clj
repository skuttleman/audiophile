...
(defn ^:private access! [executor projects project-id user-id]
  (when (empty? (-> projects
                    (qprojects/select-project-user project-id user-id)
...
(defn ^:private create-version* [executor files file-versions file user-id file-id]
  (-> file-versions
      (entities/insert-into {:artifact-id (:artifact/id file)
                             ...
                             :created-by  user-id})
      (->> (repos/execute! executor)))
  (repos/execute! executor (qfiles/select-by files file-id)))
...
(defn create-file
  ...
  (repos/transact! repo
                   (fn [executor {:entity/keys [files file-versions projects]}]
                     (access! executor projects project-id user-id)
                     (-> files
...
(defn create-file-version
  ...
  [repo project-id file-id version user-id]
  (repos/transact! repo
                   (fn [executor {:entity/keys [files file-versions projects]}]
                     (access! executor projects project-id user-id)
                     (->> (create-version* executor
