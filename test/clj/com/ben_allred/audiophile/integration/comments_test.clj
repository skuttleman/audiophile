(ns ^:integration com.ben-allred.audiophile.integration.comments-test
  (:require
    ...
    (int/with-config [system [:api/handler#api]] {:db/enabled? true}
      (let [handler (-> system
        ...
        (testing "when authenticated as a user with comments"
          (let [user (int/lookup-user system "joe@example.com")
                ...
                response (-> {}
                             ...
                             (ihttp/get system :api/file.comments {:route-params {:file-id file-id}})
                             handler)
        ...
        (testing "when not authenticated"
          (let [response (-> {}
                             (ihttp/get system :api/file.comments {:route-params {:file-id (uuids/random)}})
                             handler)]
